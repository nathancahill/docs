...
export default () => (
  <div>
    ...
    <p>
      View our{' '}
      <Link href="/product?name=cappuccino">
        <a>cappuccino product</a>
      </Link>
      !
    </p>
  </div>
...
<Note>
  ...
  dynamic based on other factors, but for simplicity, we'll use "cappuccino" as
  the product name.
</Note>
...
export default () => (
  <div>
    <h1>Welcome to our Next.js website!</h1>
-   <p>View our <Link href="/product?name=cappuccino"><a>cappuccino product</a></Link>!</p>
+   <p>View our <Link href="/product?name=cappuccino" as="/product/cappuccino"><a>cappuccino product</a></Link>!</p>
  </div>
)
```
...
Now, clicking the cappuccino product link will result in the URL path being `/product/cappuccino` while rendering the `product` page with the name query parameter set to `cappuccino`.
